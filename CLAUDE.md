...
과제명은 **"동아리 활동 기록 관리 웹앱"** 이다. 다이어트는 **차별화용 확장**이고, 과제 자체가 아니다.
확장이 재미있어서 필수 요건을 밀어내는 순간 제출 요건에서 떨어진다.

**필수 기능 3종이 화면에서 1급이어야 한다.** 심사자가 찾아 헤매게 만들지 않는다.

1. 활동 등록 — 활동명, 날짜, **장소**, **참여 인원**, 메모
2. 활동 목록 조회 — 최신순, 빈 목록 안내 문구
3. 활동 삭제 — 개별 삭제, 확인 절차
...
| 데이터 JSON 내보내기 / 가져오기 | `Store.exportJson/importJson` |
| 반응형 레이아웃 | `style.css` |
| 활동명·장소 키워드 검색 | 시간 남으면 (가장 저렴한 추가 1개) |

체중 추이·칼로리 수지는 명세 목록 **밖의 자체 확장**이다. 위 항목들을 대체하지 않는다.

**제출물 체크 (16:10부터)**
- [ ] 루트 `README.md` — 팀원 명단과 역할 / 구현한 필수·선택 기능 목록 / 실행 방법 / **스크린샷 2장 이상**
- [ ] 루트 `CLAUDE.md`
- [ ] 커밋 5회 이상 + **팀원 전원의 커밋 기록**
- [ ] 저장소명 `hackathon-<팀번호>-<팀이름>`, 공개
- [ ] 16:40 이후 커밋은 인정되지 않는다
