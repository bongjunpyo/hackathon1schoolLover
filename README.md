...
![활동 목록과 오늘의 체중](docs/screenshot-1.png)

오늘의 체중 카드와 추이 눈금, 활동 목록(최신순 26건). 각 항목에 날짜·활동명·종목·장소·인원·운동시간·체중·칼로리와
삭제 버튼이 있다. 기록이 0건이면 `아직 기록이 없습니다. 오늘 체중부터 남겨보세요.` 안내가 대신 표시된다.

### 통계 화면

![통계와 감량 효과 랭킹](docs/screenshot-2.png)

활동 수·참여 인원 합계와 평균·운동 시간, 월별 활동 횟수, 종목별, **감량 효과 Top3(운동·음식)와 피해야 할 항목**.
증감은 색이 아니라 방향 삼각형(▼ 감량 / ▲ 증량)으로 표시하고 표본 수를 함께 적는다.
   ...
   '.                         [ 동제 ]                         .'
    ...
    |__________________________________________________________|
     '-.____________________________________________________.-'
                 ...
                 | Store.getAll()                        | Analytics.monthlyCount()
                 | Store.remove()                        | Analytics.topFoods()
                 v                                       v

   .-''''''''''''''''''''''''''-.          .-''''''''''''''''''''''''''-.
 .'                              '.      .'                              '.
(      P E R S I S T E N C E       )    (      C O M P U T A T I O N       )
 '.           [ 효민 ]           .'      '.           [ 준표 ]           .'
  |______________________________|        |______________________________|
  | js/store/store.js            |        | js/analytics/analytics.js    |
  | 스키마 / 검증 / 직렬화       |        | 순수 함수 / 통계 / 랭킹      |
  | 샘플 생성 / JSON 입출력      |        | 상태 없음 / 부수효과 없음    |
  |______________________________|        |______________________________|
   '-.________________________.-'          '-.________________________.-'

                 |                                       ^
                 | setItem / getItem                     | Activity[] 를 인자로 받는다
                 v                                       |
                 |                                       |
                 '--------------+------------------------'
                                |
...
| 파일 | 내용 |
|---|---|
| [`docs/보고서_1팀_schoolLover_4쪽.pdf`](docs/보고서_1팀_schoolLover_4쪽.pdf) | 제출용 결과 보고서 (양식 요건 2~4쪽) |
| [`docs/보고서_1팀_schoolLover_상세.pdf`](docs/보고서_1팀_schoolLover_상세.pdf) | 상세본. 데이터 아키텍처·CRUD·필드 정의를 전부 담은 12쪽 |
| [`docs/설계서.md`](docs/설계서.md) | 설계 결정 기록과 열려 있던 결정 5건 확정 |
| [`docs/AI_USAGE.md`](docs/AI_USAGE.md) | Claude Code 위임 내역과 검증 방법. 프롬프트 원문 그대로 |
